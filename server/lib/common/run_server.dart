...
import 'package:benchmark_server/common/log_printer.dart';
...
Future<void> runServer<Config>({
  ...
  l.capture<void>(
    ...
    LogOptions(
      ...
      printColors: false,
      overrideOutput: $logPrinter(parsedArguments.environment, parsedArguments.verbose),
    ),
  );
}
...
({
  ...
  final argResult = (ArgParser()
        ...
        ..addOption(
          ...
          defaultsTo:
              io.Platform.environment['ISOLATES'] ?? const String.fromEnvironment('ISOLATES', defaultValue: '0'),
        )
  ...
  static void _endpoint<Config>(SharedWorker<Config> args) {
    ...
    l.capture<void>(
      () => runZonedGuarded<void>(
        ...
        (error, stackTrace) => args.sendPort.send(<String>[error.toString(), stackTrace.toString()]),
      ),
      LogOptions(
        handlePrint: true,
        outputInRelease: true,
